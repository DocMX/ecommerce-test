...
onMounted(() => {
  console.log("onMounted: Initializing component");
  if (route.params.id) {
...
function onSubmit($event, close = false) {
  console.log("onSubmit: Saving product", product.value, "Close after save:", close);
  loading.value = true
  errors.value = {};
  
  product.value.quantity = product.value.quantity || null
  if (product.value.id) {
    console.log("Updating product with ID:", product.value.id);
